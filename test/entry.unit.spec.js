const assert = require('chai').assert,
    ...
    { Entry, composeEntryCommit, composeEntryReveal, composeEntry, computeEntryTxId, composeEntryLedger } = require('../src/entry');
...
describe('Test Entry', function () {
    ...
    it('should reject invalid signature manually provided for entry commit (timestamp not fixed)', function () {
        ...
        const secretKey = sign.keyPair.fromSeed(Buffer.from('19c71085f906291070d07cb64cf160692c033d5e6ac05342f2219c03b4a5c8c6', 'hex')).secretKey;
        const signature = sign.detached(buffer, secretKey);

        try {
            composeEntryCommit(e1, 'EC2UFobcsWom2NvyNDN67Q8eTdpCQvwYe327ZeGTLXbYaZ56e9QR', signature);
        } catch (e) {
            assert.instanceOf(e, Error);
            return;
        }
